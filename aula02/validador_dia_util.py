...
def main():
    ...
    if verificar_btn:
        ...
        if dia_util:
            st.markdown(f'<div class="result-container util">O dia {data_formatada} é um dia útil.</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="result-container nao_util">O dia {data_formatada} não é um dia útil.</div>', unsafe_allow_html=True)
